...
CHERI_CPU_GIT_ROOT?=../../
CHERIROOT?=$(CHERI_CPU_GIT_ROOT)/cheri$(BERI_VER)
...
MEMCONV=$(error Cannot find find memConv.py, set CHERILIBS to the cherilibs directory or set CHERI_CPU_GIT_ROOT to the SVN root)
else
MEMCONV=python ${TOOLS_DIR_ABS}/memConv.py
endif

ifeq ($(wildcard $(CHERIROOT_ABS)),)
CHERI_SW_MEM_BIN=$(error Cannot find find CHERIROOT/sw/mem.bin, set CHERIROOT to the cheri directory or set CHERI_CPU_GIT_ROOT to the SVN root)
else
CHERI_SW_MEM_BIN=${CHERIROOT_ABS}/sw/mem.bin
endif

ifeq ($(wildcard $(MEMCONF)),)
MEMCONF=$(error Cannot find find $(CHERIROOT_ABS)/memoryconfig, set CHERIROOT to the cheri directory or set CHERI_CPU_GIT_ROOT to the SVN root )
endif

ifeq ($(wildcard $(SIM)),)
SIM_ABS=$(error Cannot find find $(SIM), set CHERIROOT to the cheri directory )
else
...
_PYTEST_NOT_INSTALLED_STR=No module named pytest
PYTEST_VERSION_OUTPUT=$(shell $(NOSETESTS) --version 2>&1)
PYTEST_CHECK1=$(if $(findstring $(_PYTEST_NOT_INSTALLED_STR),$(PYTEST_VERSION_OUTPUT)),$(error "pytest not installed? Try running 'pip3 install --user pytest'. Got error running $(NOSETESTS) --version: $(PYTEST_VERSION_OUTPUT)"),@echo pytest is installed)
...
# NOSETESTS is the same as PYTEST just with the -q flag
...
# QEMU_NOSETESTS is the same as QEMU_PYTEST just with the -q flag
QEMU_PYTEST_ENV=LOGDIR=$(QEMU_LOGDIR) TEST_MACHINE=QEMU CAP_PRECISION_BITS=$(QEMU_CHERI_CC_BASE_WIDTH)
QEMU_NOSETESTS= 	$(QEMU_PYTEST_ENV) $(NOSETESTS) $(QEMU_NOSEFLAGS)
QEMU_PYTEST=		$(QEMU_PYTEST_ENV) $(PYTEST) $(QEMU_NOSEFLAGS)
...
# SAIL_NOSETESTS is the same as SAIL_PYTEST just with the -q flag
