...
#
# Test rdhwr of register 29 (the user local register) on a multi-threaded 
# CPU.
#
# This register should be per-thread. To test that it is, each thread copies
# its thread id into the user local register, synchronized with the other
# threads, reads back the user local register, and checks that it has the
# expected per-thread value.
