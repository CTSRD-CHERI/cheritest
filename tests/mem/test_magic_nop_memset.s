...
BEGIN_TEST
		...
		dli	$a2, 4095	# all but the last byte
		ori	$zero, $zero, 0xC0DE	# call the magic helper

		move	$a4, $a0	# check that a0 wasn't changed
		move	$s2, $v0	# check return value
		move	$s3, $v1	# should be changed to 0xdeed by the helper
		dla	$a5, end_data
		dla	$a6, begin_data
		lbu	$s4, 0($a6)	# load first byte after memset
		lbu	$s5, -2($a5)	# check that the second to last byte changed
		lbu	$s6, -1($a5)	# but the last and the one after didn't
		lbu	$s7, 0($a5)
END_TEST
