...
#
# Test single-precision roundto long word when the FPU is in 64 bit mode
#

from beritest_tools import BaseBERITestCase
from nose.plugins.attrib import attr

class test_raw_fpu_round_l_s_d64(BaseBERITestCase):

    @attr('float64')
    def test_raw_fpu_round_l_s_d64_1(self):
        '''Test single precision round of -0.75'''
	self.assertRegisterEqual(self.MIPS.a0 , 0xffffffffffffffff, "-0.75 did not round down to -1")

    @attr('float64')
    def test_raw_fpu_round_l_s_d64_2(self):
        '''Test single precision round of -0.5'''
	self.assertRegisterEqual(self.MIPS.a1 , 0, "-0.5 did not round up to 0")

    @attr('float64')
    def test_raw_fpu_round_l_s_d64_3(self):
        '''Test single precision round of -0.25'''
    ...
    @attr('float64')
    def test_raw_fpu_round_l_s_d64_5(self):
