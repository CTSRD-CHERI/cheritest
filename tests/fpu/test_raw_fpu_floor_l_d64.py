...
class test_raw_fpu_floor_l_d64(BaseCHERITestCase):

    @attr('float64')
    def test_raw_fpu_floor_l_d64_1(self):
        '''Test double precision floor.l of -0.75'''
	self.assertRegisterEqual(self.MIPS.a0 & 0xffffffff, 0xffffffff, "-0.75 did not round down to -1")

    @attr('float64')
    def test_raw_fpu_floor_l_d64_2(self):
        '''Test double precision floor.l of -0.5'''
	self.assertRegisterEqual(self.MIPS.a1 & 0xffffffff, 0xffffffff, "-0.5 did not round down to -1")

    @attr('float64')
    def test_raw_fpu_floor_l_d64_3(self):
        '''Test double precision floor.l of -0.25'''
	self.assertRegisterEqual(self.MIPS.a2 & 0xffffffff, 0xffffffff, "-0.25 did not round down to -1")

    @attr('float64')
    def test_raw_fpu_floor_l_d64_4(self):
        '''Test double precision floor.l of 0.5'''
	self.assertRegisterEqual(self.MIPS.a3, 0, "0.5 did not round down to 0")

    @attr('float64')
    def test_raw_fpu_floor_l_d64_5(self):
        '''Test double precision floor.l of 1.5'''
	self.assertRegisterEqual(self.MIPS.a4, 1, "1.5 did not round down to 1")

    @attr('float64')
    def test_raw_fpu_floor_l_d64_6(self):
        '''Test floor of double precision to 64 bit int'''
